...
export default class AtkReloadViewPlugin extends AbstractPlugin {
    main() {
        ...
        if (!settings.stateContext && $(this.element).hasClass('ui modal') && $(this.element).children().length > 0 /* prevent loading in original DOM location */) {
            [settings.stateContext] = $(this.element).children('.content');
        ...
        if (!settings.stateContext && $(this.element).hasClass('atk-right-panel') && $(this.element).children().length > 0 /* prevent loading in original DOM location */) {
            [settings.stateContext] = $(this.element).children('.ui.segment:not(:has(> .atk-panel-warning))');
            if (!settings.className) {
                settings.className = [];
            }
