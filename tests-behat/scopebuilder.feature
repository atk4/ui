Feature: ScopeBuilder
  ...
  Scenario: test ScopeBuilder rendering of model scope
    ...
    Then rule "atk_fp__project_budget" operator is ">=" and value is "1000"
    Then rule "atk_fp__project_name" operator is "matches regular expression" and value is "[a-zA-Z]"
    Then select rule "atk_fp__currency" operator is "equals" and value is "USD"
    Then reference rule "atk_fp__client_country_iso" operator is "equals" and value is "Brazil"
    Then date rule "atk_fp__start_date" operator is "is on" and value is "Oct 22, 2020"
    Then date rule "atk_fp__finish_time" operator is "is not on" and value is "22:22"
    Then bool rule "atk_fp__is_commercial" has value "No"
