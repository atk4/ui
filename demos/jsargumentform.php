...
$country->addAction('test1', ['callback'=>function(){ return 'ok'; }]);

// clicking button should show preview window wiht OK. If OK is pressed should close window and display toast OK
$country->addAction('test2', ['preview'=>function(){ return 'show this on preview screen'; }, 'callback'=>function(){ return 'ok'; }]);

// clicking button no effect, because action is disabled
$country->addAction('test3', ['enabled'=>false, 'callback'=>function(){ return 'ok'; }]);

// invoking this action requires argument "age" (integer). User should be prompted, end would return age in response
$country->addAction('test4', ['args'=>['age'=>'int'], 'callback'=>function($age){ return 'age='.$age; }]);
...
$country->addAction('test5', ['fields'=>['iso3'], 'callback'=>function(){ return 'ok'; }]);

// if action throws exception, need to properly display to user
$country->addAction('test6', ['callback'=>function(){ throw new \atk4\ui\Exception('ouch'); }]);
...
class jsAction extends \atk4\ui\jsExpression {
    function __construct(atk4\data\UserAction\Generic $action)
    {
        if ($action->args){
            // popup code
        }
