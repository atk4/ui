...
$form->onSubmit(function (Form $form) use ($output, $c, $app) {
    return $output($form->model->get($c->shortName), $app->uip->dateFormat);
});
...
$form->onSubmit(function (Form $form) use ($output, $c, $app) {
    return $output($form->model->get($c->shortName), $app->uip->timeFormat);
});
...
View::addTo($app, ['ui' => 'hidden divider']);
$app->uip->datetimeFormat = 'Y-m-d (H:i:s)';
$form = Form::addTo($app);
$c = $form->addControl('datetime', [Form\Control\Calendar::class, 'type' => 'datetime']);
$form->buttonSave->set($c->shortName);

$form->onSubmit(function (Form $form) use ($output, $c, $app) {
    return $output($form->model->get($c->shortName), $app->uip->datetimeFormat);
});
