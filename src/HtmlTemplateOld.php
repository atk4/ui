 ...
 */
class HtmlTemplateOld implements \ArrayAccess
{
    ...
    {
        $res = [];
        foreach ($this->get($region) as $k => $v) {
            if (is_string($v)) {
                $res[] = $v;
            } elseif (is_array($v)) {
                $tag = explode('#', $k, 2)[0];
                ...
                throw (new Exception('Value has no save support'))
                    ->addMoreInfo('value_debug_type', get_debug_type($v));
            }
        }
    ...
    public function getTagTree(string $tag): HtmlTemplate\TagTree
    {
        return (new HtmlTemplateNew($this->toLoadableString($tag)))->getTagTree(HtmlTemplateNew::TOP_TAG);
    }
}
